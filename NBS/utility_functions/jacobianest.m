function [jac,err] = jacobianest(fun,x0,args)
...
for i = 1:nx
  ...
  fdel = zeros(n,nsteps);
  for j = 1:nsteps
    fdif = call_fun(swapelement(x0,i,x0_i + delta(j)),args,fun) - ...
      call_fun(swapelement(x0,i,x0_i - delta(j)),args,fun);
...
function f = call_fun(x,args,fun)
    args{2} = x;
    f = fun(args{:});
end
